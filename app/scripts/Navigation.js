var SERVER_URL = 'http://cs2356.cs.smu.ca:7973';
...
{
  var currValPasscode=$("#passcode").val();
  var currValEmailcode=$("#emailcode").val();

  if(button=="bksp")
  {
    $("#passcode").val(currValPasscode.substring(0,currVal.length-1));
    $("#passcode").val(currValEmailcode.substring(0,currVal.length-1));
  }
  else
  {
    $("#passcode").val(currValPasscode.concat(button));
  }
}
...
$( "#btnEnter" ).click(function()
{
  ...
  $.post(SERVER_URL + '/login', loginCredentials, function(data) {
    if(data && data.email==loginCredentials.email && data.password==loginCredentials.password) {
      sessionStorage.password = $("#passcode").val();
      sessionStorage.email = $("emailcode").val();
